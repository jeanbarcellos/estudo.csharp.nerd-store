...
namespace NerdStore.WebApp.MVC.Tests.Config
{
    ...
    // Classe principal que fará a instrumentação do Selenium
    public class SeleniumHelper : IDisposable
    {
        ...
        public SeleniumHelper(Browser browser, ConfigurationHelper configuration, bool headless = true)
        {
            ...
            WebDriver.Manage().Window.Maximize(); // Maximizar janela do browser
            Wait = new WebDriverWait(WebDriver, TimeSpan.FromSeconds(30)); // Definir o tempo de espera
        }
